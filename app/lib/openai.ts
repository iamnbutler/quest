...
async function getChatCompletion(messages: ChatCompletionRequestMessage[]): Promise<CreateChatCompletionResponse> {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Fetch to api/completion failed with status: ${response.status}`);
        }
