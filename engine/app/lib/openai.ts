...
async function buildMessages(message: ChatCompletionRequestMessage) {
    ...
    const { currentMessage } = useMessagesStore();
    const currentMessageContent = currentMessage.message.content.join("\n");
...
const formattingInstructions = `Based on the current situation in the story and the choice the user provides:

First, Please provide a description of the location, including the name of the province and area, separated by the delimiter "|", and wrapped in the delimiter "||". For example:
...
Then write an interesting continuation of the story, then provide 2-5 questions or actions the player can choose from to continue the story. List the options with numbers, followed by a colon, and then the option text.
